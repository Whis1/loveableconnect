-- 🔧 Fix: 'Database error querying schema' al login admin appena creato.
--
-- Causa: in auth.identities il campo provider_id per provider='email' deve
-- essere l'EMAIL stessa, non l'user_id come stringa. GoTrue moderno usa
-- provider_id+provider come chiave univoca per identificare l'identità.
-- Inserendo user_id::text al posto dell'email rompevamo il lookup di login.
--
-- Inoltre aggiorno gli identities esistenti creati con la RPC bugged finora.
...
BEGIN
  ...
  -- 🆕 provider_id = email (non user_id::text!). GoTrue lo usa per
  -- identificare l'identità in fase di login.
  INSERT INTO auth.identities (
    id, provider_id, user_id, identity_data, provider,
    last_sign_in_at, created_at, updated_at
  ) VALUES (
    gen_random_uuid(),
    v_email,           -- ✅ email, non user_id
    v_user_id,
    jsonb_build_object(
      'sub', v_user_id::text,
      'email', v_email,
...
-- 🛠 RIPARA gli identities GIA' CREATI con provider_id sbagliato (user_id::text).
-- Li aggiorna mettendo l'email corretta presa da auth.users.
