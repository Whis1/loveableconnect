...
serve(async (req) => {
  ...
  try {
    ...
    const { recipient_id, match_id } = await req.json();
    ...
    // Verifica se esiste già un customer per il gifter
    ...
    const chatPath = match_id ? `/chat/${match_id}` : "/matches";
    ...
    // Crea record del regalo in pending
    ...
    if (giftError) {
      logStep("ERROR creating gift record", { error: giftError });
      throw giftError;
    }

    logStep("Gift record created successfully");
