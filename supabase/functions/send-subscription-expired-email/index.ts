...
serve(async (req) => {
  ...
  try {
    ...
    const variables = {
      ...
      subscriptionType: previousSubscriptionType,
      planName: previousSubscriptionType,
    };

    const subject = tmpl ? replaceTemplateVars(tmpl.subject, variables) : "Il tuo abbonamento è scaduto 😢";
          ...
          <body style="margin:0; padding:0; background:linear-gradient(135deg,#fde2e4,#f3e8ff); font-family:'Segoe UI',Roboto,Arial,sans-serif;">
            <div style="max-width:600px; margin:40px auto; background:white; border-radius:20px; overflow:hidden; box-shadow:0 10px 40px rgba(0,0,0,0.1);">
              ...
              <div style="padding:40px 30px;">
                <p>Il tuo abbonamento ${previousSubscriptionType} è scaduto.</p>
                <p>Rinnova ora per continuare a godere dei vantaggi premium!</p>
              </div>
            </div>
          </body>
        </html>`;
