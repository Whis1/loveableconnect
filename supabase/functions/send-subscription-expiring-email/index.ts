...
serve(async (req) => {
  ...
  try {
    ...
    const formattedDate = new Date(expiresAt).toLocaleDateString('it-IT', {
      day: 'numeric',
      month: 'long',
      year: 'numeric'
    });
    ...
    const { data: tmpl } = await supabase
      ...
      .select('*')
      .eq('template_key', 'subscription_expiring')
      .maybeSingle();
    ...
    const subject = tmpl ? replaceTemplateVars(tmpl.subject, variables) : `Il tuo abbonamento scade tra ${daysRemaining} giorni ⏰`;
          ...
          <body style="margin:0; padding:0; background:linear-gradient(135deg,#fde2e4,#f3e8ff); font-family:'Segoe UI',Roboto,Arial,sans-serif;">
            <div style="max-width:600px; margin:40px auto; background:white; border-radius:20px; overflow:hidden; box-shadow:0 10px 40px rgba(0,0,0,0.1);">
              ...
              <div style="padding:40px 30px;">
                <p>Il tuo abbonamento ${subscriptionType} scadrà tra <strong>${daysRemaining} giorni</strong> il ${formattedDate}.</p>
                <p>Rinnova ora per non perdere i vantaggi premium!</p>
              </div>
            </div>
          </body>
        </html>`;
